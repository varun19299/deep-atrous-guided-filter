...
class GCANet_atrous(nn.Module):
    ...
    def forward(self, x):
        ...
        # TODO: this is problematic. Should have been y4?
        y3 = self.res_final(y)
...
class GCANet_atrous_corrected(nn.Module):
    def __init__(self, in_c=4, out_c=3, args=None):
        super().__init__()
...
def main(_run):
    ...
    # model = GCANet_atrous(in_c=12, out_c=12, args=args).to(args.device)
    model = GCANet_atrous_corrected(in_c=12, out_c=12, args=args).to(args.device)
